...
#my $plot_clean=decode_entities($movie_xml->{OpenSearchDescription}->{movies}->{movie}->{overview}->{value});
	my $plot_clean=$movie_xml->{OpenSearchDescription}->{movies}->{movie}->{overview}->{value};
